...
def fetch_images_recursive(pool_sema: threading.Semaphore, img_sema: threading.Semaphore, keyword: str,
                           output_dir: str, filters: str, limit: int, current=1):
    global tried_urls
    global image_md5s
    global in_progress
    global urlopenheader

    if len(tried_urls) >= limit:
        return
    ...
    try:
        ...
        fetch_images_recursive(pool_sema, img_sema, keyword, output_dir, filters, limit, current)
...
def fetch_images_from_keyword(pool_sema: threading.Semaphore, img_sema: threading.Semaphore, keyword: str,
    ...
    fetch_images_recursive(pool_sema, img_sema, keyword, output_dir, filters, limit)
...
def main():
    ...
    limit = 2000  # Set the limit to 2000 images
    if not args.limit:
        args.limit = limit
    else:
        args.limit = min(args.limit, limit)
